...
const Dashboard = () => {
  ...
  // Keep ref in sync for use inside socket closure
  useEffect(() => {
    hospitalIdRef.current = hospitalInfo.id;
  }, [hospitalInfo.id]);

  // Socket.IO for pre-arrival alerts
  useEffect(() => {
    ...
    socket.on('connect', () => {
      ...
      if (hospitalIdRef.current) {
        ...
        console.log('[PreArrival] Connected but hospitalId not known yet — will join after data loads');
      }
    });
  ...
  // Join hospital room once hospitalId is available (handles async data load)
  useEffect(() => {
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    if (!hospitalId) {
      toast.error('Hospital data not loaded yet, please wait');
      return;
    }
    try {
      const token = TokenStorage.getToken();
  ...
  const handleCapabilityStatusChange = async (capabilityType: string, newStatus: string) => {
    ...
    try {
      ...
      toast.error('Failed to update capability status');
    }
  };
