    ...
    this.client.interceptors.response.use(
      ...
      (error) => {
        if (error.response?.status === 401) {
          ...
          // Clear it so the hospital user can log in with the correct credentials.
          console.error('[Hospital Dashboard] Access denied (403): account does not have HOSPITAL role. Clearing token.');
          TokenStorage.removeToken();
          window.location.href = '/login';
        }
